...
**Slater's Condition.** The problem is **strictly feasible**, i.e., $\exists x \in \mathrm{relint}(D)$ such that the inequality constraints strictly hold.

$$ f_i(x) < 0, i = 1,2,\dots,m $$

**Refined Slater's Condition.** If some constraints $f_j$ are affine, the requirement $f_j(x) < 0$ can be relaxed to equality $f_j(x) \le 0$.
...
Note that $h_i(x^*) = 0$ (primal equality constraint) and that $\lambda_i^* g_i(x^*) \le 0$ since $\lambda_i^* \ge 0$ and $g_i(x^*) \le 0$ (primal inequality constraint).

**Complementary Slackness.** An important conclusion is that

$$ \sum_{i=1}^m \lambda_i^* g_i(x^*) = 0, \Rightarrow \lambda_i^* g_i(x^*) = 0, i = 1,\dots,m $$

$$ \lambda_i^* \ge 0 \Rightarrow f_i(x^*) = 0 $$

Complementary slackness holds for any primal and dual optimal when strong duality holds.
...
**KKT Optimality Conditions for Convex Problems.** If the primal problem is convex, and strong duality holds, the KKT Conditions are sufficient and necessary for optimality. I.e., KKT conditions hold *if and only if* strong duality holds, and $x^*, \lambda^*, \nu^*$ are primal/dual optimal.
...
#### Example: Linear Programming with Constraints
...
$$ \mathcal{L}(x, \lambda_1, \lambda_2, \nu) = c^Tx + \lambda_1^T(x-1) + \lambda_2 (-1-x) + \nu^T(Ax - b) $$
