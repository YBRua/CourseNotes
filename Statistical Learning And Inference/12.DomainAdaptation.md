...
- TCA uses the same formulation of $K_W$ and $L$ as DIP.
- DIP must be solved with an expensive SDP solver, and TCA lowers the computation burden.
...
$$ \tilde{K} = (KK^{-1/2}\tilde{W})(\tilde{W}^T K^{-1/2}K) = KWW^TK, $$
