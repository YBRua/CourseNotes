...
- $T_i$ completes its *write phase* before $T_j$ begins its *write phase*, and $WS(T_i) \cap RS(T_j) = \emptyset$ (items written by $T_i$ will not be read by $T_j$)
  - No conflicts between $WS(T_i)$ and $RS(T_j)$ (Items read by $T_j$ will not be written by $T_i$). Possible conflicts include $(R_i,W_j)$ and $(W_i, W_j)$, but the flow is one-way.
