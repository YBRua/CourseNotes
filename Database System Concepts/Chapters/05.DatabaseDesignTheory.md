...
- If each member of $F$ can be tested on one of the decomposed relation, then the decomposition is privacy preserving (sufficient, but not necessary)
- Compute $F^+$, and let $G$ be the union of FDs in $F^+$ that are covered by $R=\{R_1,\dots,R_n\}$. Then compute $G^+$. If $F^+ = G^+$, then the decomposition is privacy preserving
