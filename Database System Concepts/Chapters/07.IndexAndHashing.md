...
- A **bucket** is a unit of storage containing one or more entries
  - A bucket is typically a database page
...
- Periodic Rehashing
  - Create a larger (e.g. 2 times the size of the old table) hash table and rehash all entries to the new table
- Linear Hashing
  - Do rehashing in an incremental manner
- Extensible Hashing
  - Increase the number of entries in buckets, not the number of buckets
